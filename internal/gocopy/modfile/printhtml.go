...
// FormatHTML is a modified version of Format()
// but emits an HTML representation of a go.mod file.
func FormatHTML(f *FileSyntax, linkMap map[string]string) []byte {
	if linkMap == nil {
		linkMap = map[string]string{}
	}
	pr := &htmlprinter{linkMap: linkMap}
	pr.file(f)
	return pr.Bytes()
}
...
type htmlprinter struct {
	...
	linkMap      map[string]string // map of modules to their links
}

// printf prints to the buffer.
func (p *htmlprinter) printf(format string, args ...interface{}) {
	fmt.Fprintf(p, format, args...)
}

// indent returns the position on the current line, in bytes, 0-indexed.
func (p *htmlprinter) indent() int {
	b := p.Bytes()
	n := 0
...
func (p *htmlprinter) newline() {
	if len(p.comment) > 0 {
		...
		for i, com := range p.comment {
			...
			p.printf("<i>%s</i>", strings.TrimSpace(com.Token))
		}
...
func (p *htmlprinter) file(f *FileSyntax) {
	p.printf(`<pre class="GoModContainer">`)
...
func (p *htmlprinter) expr(x Expr) {
	...
	case *Line:
		sep := ""
		for idx, tok := range x.Token {
			if idx == 0 && oneOf(tok) {
				p.printf("<span>%s</span>", tok)
			} else {
				item := tok
				if link, ok := p.linkMap[item]; ok {
					item = fmt.Sprintf(`<a href="%s">%s</a>`, link, tok)
				}
				p.printf("%s%s", sep, item)
			}
			sep = " "
		}
